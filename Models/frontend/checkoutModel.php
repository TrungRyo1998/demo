	...
	trait checkoutModel{
		...
		public function insert_order_detail($customer_id, $order_id){
			...
			foreach($_SESSION['cart'] as $key=>$product){
				$query = $conn->prepare("insert into tbl_order_detail set order_id=:order_id, product_id=:product_id, price=:price");
				$query->execute(array("order_id"=>$order_id,"product_id"=>$product["id"],"price"=>$product["price"]));
			}
		}
	}
 ?>